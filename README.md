...
1. `cd` into the module's folder (e.g. `cd non-prod/ap-southeast-1/qa/mysql`).
...
1. `cd` into the region folder (e.g. `cd non-prod/ap-southeast-1`).
...
elb_dns_name = webserver-example-prod-1234567890.ap-southeast-1.elb.amazonaws.com
elb_security_group_id = sg-fe62ee8f
url = http://webserver-example-prod-1234567890.ap-southeast-1.elb.amazonaws.com:80
```
...
```
curl http://webserver-example-prod-1234567890.ap-southeast-1.elb.amazonaws.com:80
...
arn = arn:aws:rds:ap-southeast-1:1234567890:db:tofu-00d7a11c1e02cf617f80bbe301
db_name = mysql_prod
endpoint = tofu-1234567890.abcdefghijklmonp.ap-southeast-1.rds.amazonaws.com:3306
```

You can use the `endpoint` and `db_name` outputs with any MySQL client:

```
mysql --host=tofu-1234567890.abcdefghijklmonp.ap-southeast-1.rds.amazonaws.com:3306 --user=admin --password mysql_prod
```
...
- **Region**: Within each account, there will be one or more [AWS
  ...
  `ap-southeast-1`, `eu-west-1`, and `ap-southeast-2`, where you've deployed resources. There may also be a `_global`
